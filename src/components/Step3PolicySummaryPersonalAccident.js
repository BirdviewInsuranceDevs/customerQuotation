...
const Step3PolicySummaryPersonalAccident = ({setPersonalAccidentTotalPremium}) => {
    ...
    useEffect(() => {
        ...
        if (personalAccidentActive) {
            ...
            const personalAccident = data.personalAccident;
            setCoverAmount(personalAccident.coverAmount);
    
            const policyStartDate = new Date(personalAccident.policyStartDate);
            setPolicyStartDate(policyStartDate.toISOString().split('T')[0]);
    
            // Calculate end policy date
            const startDate = new Date(personalAccident.policyStartDate);
            const endDate = new Date(startDate);
            endDate.setFullYear(startDate.getFullYear() + 1);
            setEndPolicyStartDate(endDate.toISOString().split('T')[0]); // format as 'YYYY-MM-DD'
    
            // Get the holder's Premium amount
            let totalPremium = personalAccident.premium || 0; // Ensure it's a number
            ...
            dependants.forEach(dep => {
               // Enhanced handling for empty string
                const dependantPremium = dep?.personalAccidentData?.premium ? 
                (Number(dep?.personalAccidentData?.premium) || 0) : 0;
                ...
                if (dep.medicalConditions && dep.medicalConditions.length > 0) {
                    ...
                    // If no medical conditions, just add the dependant premium
            ...
            setPersonalAccidentTotalPremium(Number(totalPremium) + Number(stampDuty * paymentCurrencyRate) + Number(ipcfAmount) + Number(itl));
        }
    ...
    return (
        ...
        <div className="p-4 bg-white shadow-md shadow-div  rounded-md m-2  ">
            <h3 className="sm:text-sm md:text-lg font-semibold mb-4">Policy Summary: Last Expense</h3>
            ...
            <div className=" ">
                <div className="flex flex-col sm:flex-row justify-between items-start border-b border-gray-300 pb-2">
                    ...
                    <div className="sm:w-full md:w-2/3">Last Expense</div>
                </div>
                <div className="flex flex-col sm:flex-row justify-between items-start border-b border-gray-300 pb-2">
...
export default Step3PolicySummaryPersonalAccident;
