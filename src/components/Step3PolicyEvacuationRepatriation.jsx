...
const Step3PolicyEvacuationRepatriation = ({setEvacuationRepatriationtotalPremium}) => {
    ...
    useEffect(() => {
        ...
        if (evacuationRepatriationActive) {
            ...
            const evacuationRepatriation = data.evacuationRepatriation;
            setCoverAmount(evacuationRepatriation.coverAmount);
    
            const policyStartDate = new Date(evacuationRepatriation.policyStartDate);
            setPolicyStartDate(policyStartDate.toISOString().split('T')[0]);
    
            // Calculate end policy date
            const startDate = new Date(evacuationRepatriation.policyStartDate);
            const endDate = new Date(startDate);
            endDate.setFullYear(startDate.getFullYear() + 1);
            setEndPolicyStartDate(endDate.toISOString().split('T')[0]); // format as 'YYYY-MM-DD'
    
            // Get the holder's Premium amount
            let totalPremium = evacuationRepatriation.premium || 0; // Ensure it's a number
            ...
            dependants.forEach(dep => {
               // Enhanced handling for empty string
                const dependantPremium = dep?.evacuationData?.premium ? 
                (Number(dep?.evacuationData?.premium) || 0) : 0;
    ...
    return (
        ...
        <div className="p-4 bg-white shadow-md shadow-div  rounded-md m-2  ">
            <h3 className="sm:text-sm md:text-lg font-semibold mb-4">Policy Summary:  Evacuation And Repatriation </h3>
            ...
            <div className=" ">
                <div className="flex flex-col sm:flex-row justify-between items-start border-b border-gray-300 pb-2">
                    ...
                    <div className="sm:w-full md:w-2/3"> Evacuation And Repatriation</div>
                </div>
                <div className="flex flex-col sm:flex-row justify-between items-start border-b border-gray-300 pb-2">
...
export default Step3PolicyEvacuationRepatriation;
