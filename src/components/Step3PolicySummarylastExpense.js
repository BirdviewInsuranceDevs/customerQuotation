...
const Step3PolicySummarylastExpense = ({setLastExpenseTotalPremium}) => {
        ...
        if (lastExpenseActive) {
            ...
            dependants.forEach(dep => {
               // Enhanced handling for empty string
                const dependantPremium = dep?.lastExpenseData?.premium ? 
                (Number(dep?.lastExpenseData?.premium) || 0) : 0;
            ...
            setLastExpenseTotalPremium(Number(totalPremium) + Number(stampDuty * paymentCurrencyRate) + Number(ipcfAmount) + Number(itl));
        }
    }, [setPremium, stampDuty, setIpcf, setItl, itl,setLastExpenseTotalPremium]);
...
export default Step3PolicySummarylastExpense;
