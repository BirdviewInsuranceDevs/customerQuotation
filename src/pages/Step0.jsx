...
const Step0 = () => {
  ...
  const [checkedAddProductItems, setCheckedAddProductItems] = useState(
    savedCheckedAddProductItems
  );

  const isAnyProductSelected = () => {
    return Object.values(checkedAddProductItems).some(
      (value) => value === true
    );
  };
  ...
  const savedContactAndLoginsAndCurrency =
    savedStep1Data.contactAndLoginsAndCurrency || {
      firstName: "",
      currency: "KES",
      email: "",
    };

  const [contactAndLoginsAndCurrency, setContactAndLoginsAndCurrency] =
    useState(savedContactAndLoginsAndCurrency);
  ...
  ] = useState(savedEvacuationRepatriation);
  ...
  const [formDataStep0LastExpense, setFormDataStep0LastExpense] =
    useState(savedLastExpense);
  ...
  const [formDataStep0Medical, setFormDataStep0Medical] =
    useState(savedMedical);
  ...
  const [formDataStep0HospitalCash, setFormDataStep0HospitalCash] =
    useState(savedHospitalCash);
  ...
  const [formDataStep0PersonalAccident, setFormDataStep0PersonalAccident] =
    useState(savedPersonalAccident);

  // initialize products errors step1
  const [errorsGeneralProducts, setErrorsGeneralProducts] = useState({});
  const [errorsEvacuationRepatriation, setErrorsEvacuationRepatriation] =
    useState({});
  ...
  const validateStep = () => {
    ...
    if (!contactAndLoginsAndCurrency.firstName)
      tempErrorsGeneralProducts.firstName = "First Name is required.";
    if (!contactAndLoginsAndCurrency.email)
      tempErrorsGeneralProducts.email = " Email is required.";
    ...
    if (
      ...
      if (!formDataStep0EvacuationRepatriation.coverType)
        tempErrorsEvacuationRepatriation.coverType = "Cover Type is required.";
      // Date validation
      let formattedPolicyStartDate =
        formDataStep0EvacuationRepatriation.policyStartDate;
      // Ensure the policyStartDate is not null and is a valid Dayjs object
      if (!formattedPolicyStartDate) {
        // Handle null case
        tempErrorsEvacuationRepatriation.policyStartDate =
          "Policy Start Date is required.";
      } else if (!dayjs(formattedPolicyStartDate).isValid()) {
        // Handle invalid date format
        tempErrorsEvacuationRepatriation.policyStartDate =
          "Policy Start Date is invalid.";
      } else {
        // Convert to 'YYYY-MM-DD' for comparison
        formattedPolicyStartDate = dayjs(formattedPolicyStartDate).format(
          "YYYY-MM-DD"
        );
      ...
      if (formDataStep0EvacuationRepatriation.selectedPlan === null) {
        tempErrorsEvacuationRepatriation.selectedPlan =
          "You must select a plan before proceeding.";
      }
    }
    ...
    if (activeStep === 0 && checkedAddProductItems.lastExpense === true) {
      // Reseting the stored value in Step 0
      step1RefStoredData.current = {};
      if (!formDataStep0LastExpense.coverType)
        tempErrorsLastExpense.coverType = "Cover Type is required.";
      // Date validation
      let formattedPolicyStartDate = formDataStep0LastExpense.policyStartDate;
      // Ensure the policyStartDate is not null and is a valid Dayjs object
      if (!formattedPolicyStartDate) {
        // Handle null case
        tempErrorsLastExpense.policyStartDate =
          "Policy Start Date is required.";
      } else if (!dayjs(formattedPolicyStartDate).isValid()) {
        // Handle invalid date format
      ...
      if (formDataStep0LastExpense.selectedPlan === null) {
        tempErrorsLastExpense.selectedPlan =
          "You must select a plan before proceeding.";
      }
    }
    ...
    if (activeStep === 0 && checkedAddProductItems.medical === true) {
      // Reseting the stored value in Step 0
      step1RefStoredData.current = {};
      if (!formDataStep0Medical.coverType)
        tempErrorsLastExpense.coverType = "Cover Type is required.";
      if (formDataStep0Medical.dependantCount > 10)
        tempErrorsMedical.dependantCount =
          "Dependant Count should not exceed 10";
      // Check if the dependants count is a number and greater than 0
      if (
        formDataStep0Medical.dependantCount &&
        (isNaN(formDataStep0Medical.dependantCount) ||
          formDataStep0Medical.dependantCount < 0)
      ) {
        tempErrorsMedical.dependantCount =
          "Dependant Count must be a number greater than 0.";
      }
      ...
      if (!formattedPolicyStartDate) {
        ...
        // Check if the date is in the past
        if (formattedPolicyStartDate < today) {
          tempErrorsMedical.policyStartDate =
            "Policy Start Date cannot be in the past.";
        }
      }
      ...
      if (formDataStep0Medical.selectedPlan === null) {
        tempErrorsMedical.selectedPlan =
          "You must select a plan before proceeding.";
      }
    }
    ...
    if (activeStep === 0 && checkedAddProductItems.hospitalCash === true) {
      ...
      step1RefStoredData.current = {};
      if (
        formDataStep0HospitalCash.coverType === "per-family" &&
        !formDataStep0HospitalCash.dependantCount
      )
        tempErrorsHospitalCash.dependantCount =
          "Dependant Count is required for Per-family Cover.";
      // dependents counts should not Exceed 10
      if (formDataStep0HospitalCash.dependantCount > 10)
        tempErrorsHospitalCash.dependantCount =
          "Dependant Count should not exceed 10";
      // Check if the dependants count is a number and greater than 0
      if (
        formDataStep0HospitalCash.dependantCount &&
        (isNaN(formDataStep0HospitalCash.dependantCount) ||
          formDataStep0HospitalCash.dependantCount < 0)
      ) {
        tempErrorsHospitalCash.dependantCount =
          "Dependant Count must be a number greater than 0.";
      }
      ...
      if (!formDataStep0HospitalCash.coverType)
        tempErrorsHospitalCash.coverType = "Cover Type is required.";
      ...
      if (formDataStep0HospitalCash.selectedPlan === null) {
        tempErrorsHospitalCash.selectedPlan =
          "You must select a plan before proceeding.";
      }
    }
    ...
    if (activeStep === 0 && checkedAddProductItems.personalAccident === true) {
      // Reseting the stored value in Step 0
      step1RefStoredData.current = {};
      if (!formDataStep0PersonalAccident.coverType)
        tempErrorsPersonalAccident.coverType = "Cover Type is required.";
      // Date validation
      let formattedPolicyStartDate =
        formDataStep0PersonalAccident.policyStartDate;
      // Ensure the policyStartDate is not null and is a valid Dayjs object
      if (!formattedPolicyStartDate) {
        // Handle null case
        tempErrorsPersonalAccident.policyStartDate =
          "Policy Start Date is required.";
      } else if (!dayjs(formattedPolicyStartDate).isValid()) {
        // Handle invalid date format
        tempErrorsPersonalAccident.policyStartDate =
      ...
      if (formDataStep0PersonalAccident.selectedPlan === null) {
        tempErrorsPersonalAccident.selectedPlan =
          "You must select a plan before proceeding.";
      }
    }
    ...
    return (
      Object.keys(tempErrorsGeneralProducts).length === 0 &&
      ...
      if (activeStep === 0) {
        ...
        if (checkedAddProductItems.evacuationRepatriation) {
          const evacuationRepatriationFormData = {
            ...
            totalAmount: formDataStep0EvacuationRepatriation.totalAmount,
          };
        ...
        if (checkedAddProductItems.lastExpense) {
          const lastExpenseFormData = {
            ...
            totalAmount: formDataStep0LastExpense.totalAmount,
          };
        ...
        if (checkedAddProductItems.medical) {
          const medicalFormData = {
            ...
            totalAmount: formDataStep0Medical.totalAmount,
          };
        ...
        if (checkedAddProductItems.hospitalCash) {
          const hospitalCashFormData = {
            ...
            totalAmount: formDataStep0HospitalCash.totalAmount,
          };
        ...
        if (checkedAddProductItems.personalAccident) {
          const personalAccidentFormData = {
            ...
            totalAmount: formDataStep0PersonalAccident.totalAmount,
          };
        ...
        addFormData(
          ...
          contactAndLoginsAndCurrencyData
        );
        ...
        localStorage.setItem(
          "step1",
          JSON.stringify(step1RefStoredData.current)
        );
      }
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-2 ">
              ...
              <div className="flex-1">
                <TextField
                  ...
                  onChange={(e) => {
                    setContactAndLoginsAndCurrency({
                      ...contactAndLoginsAndCurrency,
